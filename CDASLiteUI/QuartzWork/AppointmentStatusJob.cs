...
namespace CDASLiteUI.QuartzWork
{
    ...
    public class AppointmentStatusJob : IJob
    {
        ...
        public AppointmentStatusJob(ILogger<AppointmentStatusJob> logger, IUnitOfWork unitOfWork)
        {
            _logger = logger;
        ...
        public Task Execute(IJobExecutionContext context)
        {
            try
            {
                ...
                foreach (var item in appointments)
                {
                    ...
                    if (item.AppointmentDate < DateTime.Now || (item.AppointmentDate.ToShortDateString() == DateTime.Now.ToShortDateString() && (itemHour < DateTime.Now.Hour || (itemHour == DateTime.Now.Hour && itemMinute < DateTime.Now.Minute))))
                    {
                        item.AppointmentStatus = AppointmentStatus.Passive;
                        _unitOfWork.AppointmentRepository.Update(item);
